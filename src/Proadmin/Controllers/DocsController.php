...
class DocsController extends \App\Http\Controllers\Controller
{
	...
	{
		$collections = Collection::get()->whereNotIn('table_name', ['roles', 'users']);
		...
		foreach ($collections as $collection) {
			...
			$fieldsRequired = $collection->getFieldsRequired();
			$fieldsTypes = $collection->getFieldsType();
			...
			$docs[$collection->title] = [
				[
					'method'		=> 'GET',
					'endpoint'		=> '/api/'.$collection->table_name,
					'description'	=> 'Get all '.Str::replace('_', ' ', Str::plural($collection->table_name)).' with pagination',
					'fields'		=> [
						[
							'title'		=> 'sort',
							'required'	=> 'nullable',
				...
				[
					'method'		=> 'GET',
					'endpoint'		=> '/api/'.$collection->table_name.'/{id}',
					'description'	=> 'Get single '.Str::replace('_', ' ', Str::singular($collection->table_name)).' by id',
					'fields'		=> [
						[
							'title'		=> 'fields',
							'required'	=> 'nullable',
				...
				[
					'method'		=> 'POST',
					'endpoint'		=> '/api/'.$collection->table_name,
					'description'	=> 'Create '.Str::replace('_', ' ', Str::singular($collection->table_name)),
					'fields'		=> $formatedFieldsPost,
				],
				[
					'method'		=> 'PUT',
					'endpoint'		=> '/api/'.$collection->table_name.'/{id}',
					'description'	=> 'Update '.Str::replace('_', ' ', Str::singular($collection->table_name)).' by id',
					'fields'		=> $formatedFieldsPut,
				],
				[
					'method'		=> 'DELETE',
					'endpoint'		=> '/api/'.$collection->table_name.'/{id}',
					'description'	=> 'Delete '.Str::replace('_', ' ', Str::singular($collection->table_name)).' by id',
					'fields'		=> [],
				],
			];
		}
