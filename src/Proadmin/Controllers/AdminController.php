...
class AdminController extends \App\Http\Controllers\Controller
{
	...
	public function getMenu()
	{
		...
		$tables = ['orders', 'callback_modal', 'callback_contacts', 'orders_one_click']; //name tables for count in sidebar
		foreach ($tables as $table) {

			if ($collections->where('table_name', $table)->first()){

				$table = $table;
				if ($collections->where('table_name', $table)->first()->multilanguage == 1){
		...
		foreach ($input['fields'] as $field) {
			...
			if ($field->type == 'relationship') {
				...
				if ($field->relationship_count == 'single') {
                    ...
                    $fieldTitle = 'id_'.$field->relationship_table_name;
	...
	public function getMainpage()
	{
		...
		if (Schema::hasTable('orders_product') &&
			...
			$alldata = [
				...
				'callbackall' 	=> DB::table('orders_one_click')->count() + DB::table('callback_contacts')->count(),
		...
		if (Schema::hasTable('orders_one_click') &&
			Schema::hasTable('callback_contacts')) {

			$callbacks = DB::table('orders_one_click')
			->select('created_at')
			->where('created_at','>', $lastweek)
			->get();
