...
use Probytech\Proadmin\Models\SinglePage;
...
class AdminController extends \App\Http\Controllers\Controller
{
	...
	public function admin()
	{
		...
		$customComponentsPath = resource_path('views/vendor/proadmin/components/custom');
        $customComponents = [];

        if (is_dir($customComponentsPath)) {

            $customComponentsFiles = scandir($customComponentsPath);

            foreach ($customComponentsFiles as $customComponent) {
                $pathinfo = pathinfo($customComponent);
	...
	public function getMenu()
	{
		...
		$tables = ['orders', 'callback_modal', 'callback_contacts', 'orders_one_click', 'test']; //name tables for count in sidebar
		foreach ($tables as $table) {

            $collectionItem = $collections->where('table_name', $table)->first();

            if ($collectionItem){

				$itemsTable = $table;

				if ($collectionItem->multilanguage == 1){
					$itemsTable = $table.'_'.Lang::get();
				}

				$count = DB::table($itemsTable)
				->count();

				$collections->where('table_name', $table)->first()->count = $count;
			}
		}

		foreach ($dropdown as &$dd){
			$dd->count = $collections->where('dropdown_id', $dd->id)->sum('count');
		}
	...
	public function dbSelect()
	{
		...
		->when(!empty($join), function($q) use ($join, $table, $full_table){
			foreach (json_decode($join) as $tbl) {
				$q->leftJoin($tbl->full, "{$tbl->full}.id", "$full_table.{$tbl->short}_id");
			}
		})
		->offset($offset)
		->limit($limit)
		...
		if ($relationships != '') {
			...
			foreach ($rels as $rel) {
				...
				->select($rel_connected_table_title.'_id as rel_id')
				->where($table.'_id', $rel_id)
				->orderBy('id', 'ASC')
				->get();
	...
	{
		$field_properties = Collection::get()
			...
			foreach ($editable as $table_name) {
				...
				foreach ($tables as $tbl_index => $tbl){
					...
					foreach ($editable_items as $key => &$editable_item){
						...
						$editable_item[$table. '_id'] = $new_id;
		...
		foreach ($input['fields'] as $field) {
			...
			if ($field->type == 'relationship') {
				...
				if ($field->relationship_count == 'single') {
                    ...
                    $fieldTitle = $field->relationship_table_name.'_id';

					if ($menu->multilanguage == 1 && $field->lang == 0) {

						$update_multilanguage[$fieldTitle] = $field->value;
					}

					$update[$fieldTitle] = $field->value;
				} // other rels are below

			} else if ($field->type == 'gallery' || $field->type == 'files') {
		...
		foreach ($input['fields'] as $field) {
			...
			if ($field->type == 'relationship') {
				...
				if ($field->relationship_count == 'many') {
					...
					$col_first = $input['table'].'_id';
					$col_last = $field->relationship_table_name.'_id';
					...
					->select('id')
					->where($input['table'].'_id', $input['id'])
					->get()
					->pluck('id');
			...
			if ($field->type == 'relationship') {
				...
				if ($field->relationship_count != 'editable') {
					...
					if ($field->relationship_table_name == 'filter_fields'){
						...
						->select('id', 'title_ua', 'id_filters')
						->get();

						$filters = DB::table($this->tableService->getTable(
							'filters',
							$input['language']
						))
						->select('id', 'title_ua')
						...
						foreach ($field->values as &$value){
							...
							if ($filter)
								$value->title = $filter->title_ua.': '.$value->title_ua;
						}
					}
					...
					if ($field->relationship_count == 'single') {
                        ...
                        $field->values = DB::table($this->tableService->getTable(
                            str_replace('_other', '', $field->relationship_table_name),
                            $input['language']
                        ))
                        ->select('id', $field->relationship_view_field.' as title')
                        ->get();
						...
						if ($input['id'] == 0) {
							...
							$field->value = $instance->$fieldTitle;
                            $field->field = $fieldTitle;
						...
						if ($input['id'] == 0) {
							...
							->select($rel_connected_table.'_id'.' AS id')
							->where($input['table'].'_id', $input['id'])
							->orderBy('id', 'ASC')
							->get()
							->pluck('id');
