<template id="template-single">
	<div class="table-edit">
		<h1 class="text-xl font-bold mb-5">{{ __('Create or edit Single item') }}</h1>
		<div class="bg-white rounded-xl p-5 py-8 px-10 mb-5 flex flex-col gap-3">
			<div>
				<label class="block text-base font-medium mb-1">{{ __('Single item') }}</label>
                <div class="w-full relative">
                    <select v-on:change="setMenuItem" class="w-full text-base px-4 py-2 appearance-none cursor-pointer border border-stroke rounded-sm ring-0 focus:ring-0 outline-0 focus:outline-none">
			...
			<div>
				<label class="block text-base font-medium mb-1">{{ __('Single title') }}</label>
				<input v-model="single_item.title" class="w-full text-base px-4 py-2 appearance-none border border-stroke rounded-sm ring-0 focus:ring-0 outline-0 focus:outline-none" placeholder="{{ __('Single title (used for menu item)') }}" type="text">
            </div>
			<div>
				<label class="block text-base font-medium mb-1">{{ __('Single name  (API ID)') }}</label>
				<input v-model="single_item.slug" class="w-full text-base px-4 py-2 appearance-none border border-stroke rounded-sm ring-0 focus:ring-0 outline-0 focus:outline-none" placeholder="{{ __('ex. mainpage or prases (used to generate API)') }}" type="text" :disabled="action != 'create'">
			</div>
			<div>
				<label class="block text-base font-medium mb-1">{{ __('Sort') }}</label>
				<input v-model="single_item.sort" class="w-full text-base px-4 py-2 appearance-none border border-stroke rounded-sm ring-0 focus:ring-0 outline-0 focus:outline-none" placeholder="0" type="text">
			</div>
			<div>
				<label class="block text-base font-medium mb-1">{{ __('Dropdown') }}</label>
                <div class="w-full relative">
                    <select v-model="single_item.dropdown_id" class="w-full text-base px-4 py-2 appearance-none cursor-pointer border border-stroke rounded-sm ring-0 focus:ring-0 outline-0 focus:outline-none">
			...
			<div>
				...
				<div class="flex items-center gap-2">
					<input class="w-full text-base px-4 py-2 appearance-none border border-stroke rounded-sm ring-0 focus:ring-0 outline-0 focus:outline-none" type="text" v-model="single_item.icon">
					<div class="flex items-center gap-2">
						<img :src="single_item.icon" alt="" class="w-6 h-6 shrink-0">
		...
		<div class="bg-white rounded-xl p-5 py-8 px-10 mb-5" v-for="(block, block_index) in single_item.blocks">

			<div class="flex flex-col gap-2">
				<div class="grid grid-cols-12 gap-2 w-full">
					<div class="col-span-3 text-grey font-bold text-sm">{{ __('Block visual name') }}</div>
					<div class="col-span-3 text-grey font-bold text-sm">{{ __('Block API name') }}</div>
				</div>
				<div class="grid grid-cols-12 gap-2">
					<div class="col-span-3">
                        <input v-model="block.title" type="text" class="w-full text-base px-4 py-2 appearance-none border border-stroke rounded-sm ring-0 focus:ring-0 outline-0 focus:outline-none" placeholder="{{ __('Name') }}">
					</div>
					<div class="col-span-3">
                        <input v-model="block.slug" type="text" class="w-full text-base px-4 py-2 appearance-none border border-stroke rounded-sm ring-0 focus:ring-0 outline-0 focus:outline-none" placeholder="{{ __('Name API ID') }}">
					</div>
					<div class="col-span-6">
                        <div class="flex items-center justify-end gap-2">
                            <div v-on:click="upBlock(block_index)" class="w-6 h-6 shrink-0 flex items-center justify-center bg-blue-400 text-white text-sm font-bold rounded-lg cursor-pointer hover:bg-blue-500 duration-300">
                            ...
                            <div v-on:click="removeBlock(block_index)" class="w-6 h-6 shrink-0 flex items-center justify-center bg-red-600 text-white text-sm font-bold rounded-lg cursor-pointer hover:bg-red-500 duration-300">
		...
		<button v-on:click="addBlock()" type="button" id="addBlock" class="mb-4 border border-primary text-primary bg-light-grey hover:text-white w-full text-sm font-bold px-4 py-4 rounded-lg cursor-pointer hover:bg-hover duration-300">+ {{ __('Add block') }}</button>


        <button v-if="action == 'create'" v-on:click="editSingle()" class="bg-primary text-white text-sm font-bold px-4 py-2 rounded-lg cursor-pointer hover:bg-hover duration-300">{{ __('Create Single') }}</button>
        <div v-else class="flex items-center justify-between">
            <button v-on:click="editSingle()" class="bg-primary text-white text-sm font-bold px-4 py-2 rounded-lg cursor-pointer hover:bg-hover duration-300">{{ __('Update Single') }}</button>
            <button v-on:click="removeSingle()" class="bg-red-600 text-white text-sm font-bold px-4 py-2 rounded-lg cursor-pointer hover:bg-red-500 duration-300">{{ __('Remove Single') }}</button>
        </div>
	</div>
</template>
