<template id="template-sidebar">
	<div class="bg-white py-5 min-h-full overflow-y-auto shrink-0 flex flex-col justify-between w-[300px] -translate-x-[300px] lg:!translate-x-0 duration-300 absolute lg:relative z-50" :class="{'translate-x-0': show_mobile_menu}">

        <div class="hidden lg:flex items-center gap-4 w-full mb-4 px-5">
            <a href="/" target="_blank">
                <img src="/vendor/proadmin/images/logo.svg" alt="" class="w-20">
            </a>
            <router-link to="{{ route('admin', [], false) }}" class="bg-background text-sm xl:text-base font-bold px-4 py-2 rounded-lg">{{ __('Admin Panel') }}</router-link>
        </div>

		<div class="flex flex-col justify-between flex-1">

			<div class="flex flex-col gap-1">

				<div class="flex flex-col gap-2 pb-4 mb-2 border-b border-b-stroke px-5" v-if="is_dev">
					<div class="flex items-center gap-5" v-for="item in dev_menu">
						<svg class="w-4 h-4" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
							<path d="M15.122 6.26267L13.8687 6.10333C13.7653 5.78533 13.638 5.47867 13.4893 5.18733L14.2633 4.19067C14.5767 3.78733 14.54 3.218 14.182 2.87133L13.132 1.82133C12.782 1.46 12.2127 1.424 11.8087 1.73667L10.8133 2.51067C10.522 2.362 10.2153 2.23467 9.89667 2.13133L9.73733 0.88C9.67733 0.378 9.25133 0 8.74667 0H7.25333C6.74867 0 6.32267 0.378 6.26267 0.878L6.10333 2.13133C5.78467 2.23467 5.478 2.36133 5.18667 2.51067L4.19067 1.73667C3.788 1.424 3.21867 1.46 2.87133 1.818L1.82133 2.86733C1.46 3.218 1.42333 3.78733 1.73667 4.19133L2.51067 5.18733C2.36133 5.47867 2.23467 5.78533 2.13133 6.10333L0.88 6.26267C0.378 6.32267 0 6.74867 0 7.25333V8.74667C0 9.25133 0.378 9.67733 0.878 9.73733L2.13133 9.89667C2.23467 10.2147 2.362 10.5213 2.51067 10.8127L1.73667 11.8093C1.42333 12.2127 1.46 12.782 1.818 13.1287L2.868 14.1787C3.21867 14.5393 3.78733 14.5753 4.19133 14.2627L5.18733 13.4887C5.47867 13.638 5.78533 13.7653 6.10333 13.868L6.26267 15.1187C6.32267 15.622 6.74867 16 7.25333 16H8.74667C9.25133 16 9.67733 15.622 9.73733 15.122L9.89667 13.8687C10.2147 13.7653 10.5213 13.638 10.8127 13.4893L11.8093 14.2633C12.2127 14.5767 12.782 14.54 13.1287 14.182L14.1787 13.132C14.54 12.7813 14.5767 12.2127 14.2633 11.8087L13.4893 10.8127C13.6387 10.5213 13.766 10.2147 13.8687 9.89667L15.1193 9.73733C15.6213 9.67733 15.9993 9.25133 15.9993 8.74667V7.25333C16 6.74867 15.622 6.32267 15.122 6.26267ZM8 11.3333C6.162 11.3333 4.66667 9.838 4.66667 8C4.66667 6.162 6.162 4.66667 8 4.66667C9.838 4.66667 11.3333 6.162 11.3333 8C11.3333 9.838 9.838 11.3333 8 11.3333Z" fill="#171219"/>
						</svg>
						<router-link
                            class="text-base text-grey hover:text-primary duration-300"
                            :class="{'!text-primary': $route.path === item.link}"
                            :to="item.link"
                            v-text="item.title"
                        ></router-link>
					</div>
				</div>

				<template v-for="item in dropdown_menu">
					<div :class="{'bg-light-grey !border-r-primary': item.active}" v-if="!item.table_name" class="ml-2 pl-3 pr-5 py-1 rounded-l-md rounded-r-sm border-r-2 border-r-transparent duration-300">
						<div class="flex items-center justify-between cursor-pointer" v-on:click="item.active = !item.active">
                            <div class="flex items-center gap-5">
                                <img class="w-4 h-4" :src="item.icon" alt="" v-if="item.icon">

                                <svg v-else class="w-4 h-4" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M14.542 0.655212C14.2572 0.655212 14.0259 0.886176 14.0259 1.17133C14.0259 1.45616 14.2572 1.68745 14.542 1.68745C14.7768 1.68745 14.9678 1.87841 14.9678 2.11293V13.8862C14.9678 14.1207 14.7768 14.3117 14.542 14.3117C14.2572 14.3117 14.0259 14.543 14.0259 14.8278C14.0259 15.113 14.2572 15.3439 14.542 15.3439C15.3458 15.3439 16 14.6901 16 13.8862V2.11293C16 1.30907 15.3458 0.655212 14.542 0.655212ZM12.1501 0.655212C11.8653 0.655212 11.634 0.886176 11.634 1.17133C11.634 1.45616 11.8653 1.68745 12.1501 1.68745C12.3849 1.68745 12.5759 1.87841 12.5759 2.11293V13.8862C12.5759 14.1207 12.3849 14.3117 12.1501 14.3117C11.8653 14.3117 11.634 14.543 11.634 14.8278C11.634 15.113 11.8653 15.3439 12.1501 15.3439C12.9543 15.3439 13.6081 14.6901 13.6081 13.8862V2.11293C13.6081 1.30907 12.9543 0.655212 12.1501 0.655212ZM11.2162 2.11293V13.8862C11.2162 14.6901 10.5624 15.3439 9.75821 15.3439H4.92798C4.6425 15.3439 4.41154 15.1123 4.41186 14.8269C4.41315 14.3662 4.41121 13.4485 4.41121 11.5798C4.41121 11.2227 4.12154 10.9334 3.76478 10.9334C1.8961 10.9334 0.978377 10.9314 0.517418 10.9327C0.23194 10.9331 0.000331199 10.7021 0.000331199 10.4166V2.11293C0.000331199 1.30778 0.652899 0.655212 1.45805 0.655212H9.75821C10.5624 0.655212 11.2162 1.30907 11.2162 2.11293ZM3.91703 11.9437V14.8278C3.91703 15.2786 3.3722 15.5216 3.03576 15.193C-0.0796673 12.0772 0.0511367 12.2574 0.0100084 12.043C-0.0530226 11.7278 0.189521 11.4276 0.51645 11.4276H3.40091C3.68574 11.4276 3.91703 11.6585 3.91703 11.9437Z" fill="#171219"/>
                                    <clipPath id="clip0_749_34">
                                    <rect width="16" height="16" fill="white"/>
                                    </clipPath>
                                </svg>

                                <div class="flex items-center gap-2">
                                    <span class="text-base text-grey" :class="{'!text-primary': item.active}" v-text="item.title"></span>
                                    <span class="text-xs bg-primary text-white px-1.5 py-0.5 rounded-sm" v-text="item.count" v-if="item.count != 0"></span>
                                </div>
                            </div>
							<div class="bg-background w-5 h-5 rounded-sm flex items-center justify-center">
								<svg class="w-2 h-2 duration-300" :class="{'rotate-180': item.active}" width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
									<path d="M4.21951 6.06466L7.91359 2.37057C8.0317 2.24828 8.02831 2.0534 7.90601 1.93529C7.78671 1.82007 7.59759 1.82007 7.47831 1.93529L4.00187 5.41173L0.525423 1.93529C0.405221 1.81511 0.210343 1.81511 0.0901413 1.93529C-0.0300427 2.05551 -0.0300427 2.25037 0.0901413 2.37057L3.78422 6.06466C3.90444 6.18484 4.0993 6.18484 4.21951 6.06466Z" fill="#171219"/>
									<clipPath id="clip0_749_96">
										<rect width="8" height="8" fill="white" transform="translate(8) rotate(90)"/>
									</clipPath>
								</svg>
							</div>
						</div>
                        <div class="flex flex-col gap-2 pl-9 my-2" v-if="item.active">
                            <router-link
                                :to="(elm.type == 'multiple') ? '/admin/' + elm.table_name : '/admin/single/' + elm.table_name"
                                :key="elm.id"
                                class="text-sm text-primary"
                                v-if="is_dev || !elm.is_dev"
                                v-for="elm in item.children"
                                v-text="elm.title"
                            >
                            </router-link>
                        </div>
					</div>

					<div class="flex items-center gap-5 px-5 py-1" :class="{'active': item.active}" v-else-if="is_dev || !item.is_dev">

						<img class="w-4 h-4" :src="item.icon" alt="" v-if="item.icon">

						<svg v-else class="w-4 h-4" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
							<path d="M5.81278 0H1.21913C0.547399 0 0.000976562 0.546422 0.000976562 1.21815V5.8118C0.000976562 6.48353 0.547399 7.02995 1.21913 7.02995H5.81278C6.48451 7.02995 7.03093 6.48353 7.03093 5.8118V1.21815C7.03082 0.546422 6.48451 0 5.81278 0Z" fill="#171219"/>
							<path d="M14.7815 0H10.1879C9.51615 0 8.96973 0.546422 8.96973 1.21815V5.8118C8.96973 6.48353 9.51615 7.02995 10.1879 7.02995H14.7815C15.4533 7.02995 15.9997 6.48353 15.9997 5.8118V1.21815C15.9997 0.546422 15.4533 0 14.7815 0Z" fill="#171219"/>
							<path d="M5.8118 8.97003H1.21815C0.546422 8.97003 0 9.5164 0 10.1881V14.7818C0 15.4535 0.546422 15.9999 1.21815 15.9999H5.8118C6.48353 15.9999 7.02995 15.4535 7.02995 14.7818V10.1881C7.02984 9.5164 6.48353 8.97003 5.8118 8.97003Z" fill="#171219"/>
							<path d="M14.7815 8.97003H10.1879C9.51615 8.97003 8.96973 9.51646 8.96973 10.1882V14.7818C8.96973 15.4536 9.51615 16 10.1879 16H14.7815C15.4533 15.9999 15.9997 15.4535 15.9997 14.7818V10.1881C15.9997 9.5164 15.4533 8.97003 14.7815 8.97003Z" fill="#171219"/>
						</svg>

                        <div class="flex items-center gap-2">
                            <router-link
                                :to="(item.type == 'multiple') ? '/admin/' + item.table_name : '/admin/single/' + item.table_name"
                                :key="item.id"
                                class="text-base text-grey hover:text-primary duration-300"
                                :class="{'!text-primary': $route.path === ((item.type == 'multiple') ? '/admin/' + item.table_name : '/admin/single/' + item.table_name)}"
                                v-text="item.title"
                            ></router-link>

                            <span class="text-xs bg-primary text-white px-1.5 py-0.5 rounded-sm" v-text="item.count" v-if="item.count"></span>
                        </div>
					</div>
				</template>
				<?php foreach ($custom_components as $custom_component): ?>
					<router-link to="/admin/<?php echo $custom_component['name'] ?>" class="text-base text-grey hover:text-primary duration-300">
                        <?php echo mb_convert_case($custom_component['name'], MB_CASE_TITLE) ?>
                    </router-link>
				<?php endforeach ?>
			</div>

			<div class="px-5">
				<div class="py-2.5 border-t border-b border-stroke relative group">
                    <div class="flex items-center gap-1 cursor-pointer">
                        <div class="uppercase text-base text-grey" v-if="lang.is_active" v-for="lang in languages" v-text="lang.tag"></div>
                        <svg v-if="languages.length > 1" class="w-2 h-2 duration-300 group-hover:-rotate-90" width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M4.21951 6.06466L7.91359 2.37057C8.0317 2.24828 8.02831 2.0534 7.90601 1.93529C7.78671 1.82007 7.59759 1.82007 7.47831 1.93529L4.00187 5.41173L0.525423 1.93529C0.405221 1.81511 0.210343 1.81511 0.0901413 1.93529C-0.0300427 2.05551 -0.0300427 2.25037 0.0901413 2.37057L3.78422 6.06466C3.90444 6.18484 4.0993 6.18484 4.21951 6.06466Z" fill="#171219"/>
                            <clipPath id="clip0_749_96">
                                <rect width="8" height="8" fill="white" transform="translate(8) rotate(90)"/>
                            </clipPath>
                        </svg>
                    </div>
					<div v-if="languages.length > 1" class="absolute bg-white shadow-md px-2 py-1 flex items-center gap-2 left-10 top-1/2 -translate-y-1/2 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto duration-300">
						<div class="uppercase text-base text-grey hover:text-primary duration-300 cursor-pointer" v-if="!lang.is_active" v-for="lang in languages" v-text="lang.tag" v-on:click="set_language(lang)"></div>
					</div>
				</div>

				<a href="{{ route('admin-logout') }}" class="flex items-center gap-5 text-grey hover:text-primary duration-300 mt-4">
					<svg class="w-4 h-4" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
						<path d="M15.9493 7.07856C15.9153 6.99721 15.8667 6.92321 15.8047 6.86121L13.8053 4.86187C13.5447 4.60187 13.1233 4.60187 12.8627 4.86187C12.602 5.12253 12.602 5.54453 12.8627 5.80453L13.7247 6.66653H10.0006C9.63198 6.66653 9.33398 6.96518 9.33398 7.33318C9.33398 7.70118 9.63198 7.99984 10.0006 7.99984H13.7246L12.8626 8.86184C12.602 9.1225 12.602 9.5445 12.8626 9.8045C12.9926 9.93515 13.1633 9.99984 13.334 9.99984C13.5047 9.99984 13.6753 9.93518 13.8053 9.8045L15.8047 7.80515C15.8667 7.74381 15.9153 7.66981 15.9493 7.58781C16.0166 7.42521 16.0166 7.24121 15.9493 7.07856Z" fill="currentColor"/>
						<path d="M11.3343 9.33334C10.9656 9.33334 10.6676 9.632 10.6676 10V13.3333H8.00098V2.66666C8.00098 2.37266 7.80763 2.11266 7.52563 2.028L5.21029 1.33334H10.6676V4.66669C10.6676 5.03469 10.9656 5.33334 11.3343 5.33334C11.7029 5.33334 12.0009 5.03469 12.0009 4.66669V0.666687C12.0009 0.298656 11.7029 0 11.3343 0H0.667633C0.643633 0 0.622289 0.01 0.598977 0.0126563C0.567633 0.016 0.538977 0.0213125 0.508977 0.0286562C0.438977 0.0466563 0.375633 0.074 0.31632 0.111313C0.301664 0.120656 0.283664 0.121312 0.269664 0.131969C0.264289 0.136 0.262289 0.143344 0.256945 0.147344C0.184289 0.204656 0.123633 0.274656 0.0796328 0.358C0.0702891 0.376 0.0682891 0.395344 0.0609766 0.414C0.0396328 0.464656 0.0163203 0.514 0.00832031 0.57C0.00497656 0.59 0.0109766 0.608656 0.0103203 0.628C0.00966406 0.641344 0.000976562 0.653344 0.000976562 0.666656V14C0.000976562 14.318 0.225633 14.5913 0.536977 14.6533L7.20363 15.9867C7.24698 15.996 7.29098 16 7.33429 16C7.48695 16 7.63695 15.9474 7.75695 15.8487C7.91095 15.722 8.00095 15.5333 8.00095 15.3333V14.6667H11.3343C11.7029 14.6667 12.0009 14.368 12.0009 14V10C12.0009 9.632 11.7029 9.33334 11.3343 9.33334Z" fill="currentColor"/>
						<clipPath id="clip0_749_664">
						<rect width="16" height="16" fill="white"/>
						</clipPath>
					</svg>
					{{ __('Sign Out') }}
				</a>
			</div>
		</div>
	</div>
</template>

<script>
	Vue.component('template-sidebar',{
		template: '#template-sidebar',
		props:['is_dev', 'menu', 'dropdown', 'show_mobile_menu'],
		data: function () {
			return {
                dev_menu: [
                    {
                        title: "{{ __('Collections') }}",
                        link: "/admin/collections",
                    },
                    {
                        title: "{{ __('Single') }}",
                        link: "/admin/single",
                    },
                    {
                        title: "{{ __('Dropdown') }}",
                        link: "/admin/dropdown",
                    },
                    {
                        title: "{{ __('Settings') }}",
                        link: "/admin/settings",
                    },
                    // {
                    //     title: "{{ __('Docs') }}",
                    //     link: "/admin/docs",
                    // }
                ],
				dropdown_menu: [],
				languages: JSON.parse('<?php echo json_encode($languages); ?>'),
			}
		},
		methods: {
			generate_menu: function(){
				// init
				dropdown_menu = {}
				// set dropdown_id
				var last_id = 0
				for (var i = 0; i < this.dropdown.length; i++) {
					dropdown_menu[this.dropdown[i].id] = Object.assign({children: []}, this.dropdown[i])
					last_id = this.dropdown[i].id
				}
				for (var i = this.menu.length - 1; i >= 0; i--) {
					if (this.menu[i].dropdown_id != 0) {
						if (dropdown_menu[this.menu[i].dropdown_id]) {
							dropdown_menu[this.menu[i].dropdown_id].children.push(
								this.menu[i]
							)
						}
					} else {
						last_id++
						dropdown_menu[last_id] = this.menu[i]
					}
				}
				// rm empty dropdown
				for (var i in dropdown_menu) {
					if (!dropdown_menu[i].table_name && dropdown_menu[i].children.length == 0) {
						delete dropdown_menu[i]
					} else if (!dropdown_menu[i].table_name) {
						dropdown_menu[i].active = false
					}
				}
				// sort
				var sortable = [];
				for (var elm in dropdown_menu) {
					sortable.push([elm, dropdown_menu[elm].sort])
				}
				sortable.sort(function(a, b) {
					return a[1] - b[1];
				})

				var sorted = {}
				sortable.forEach(function(item){
					sorted[item[0]] = dropdown_menu[item[0]]
				})
				// save
				this.dropdown_menu = sorted
			},
			sub_is_active: function(input) {
				const paths = Array.isArray(input) ? input : [input]
				return paths.some(path => {
					return this.$route.path.indexOf('/admin/' + path.table_name) === 0
				})
			},
			get_language: function(){
				for (i in this.languages) {
					if (this.languages[i].is_active)
						return this.languages[i]
				}
				return null
			},
			set_language: function(lang){

				set_cookie('lang', lang.tag, 30)
				document.location.reload()
			},
		},
		watch: {
			menu: function(){
				this.generate_menu()
			},
			dropdown: function(){
				this.generate_menu()
			},
		},
		created: function(){
			var lang_tag = get_cookie('lang')

			if (!lang_tag) {
				for (i in this.languages) {
					if (this.languages[i].main_lang == 1) {

						this.languages[i].is_active = true
						set_cookie('lang', this.languages[i].tag, 30)

					} else this.languages[i].is_active = false
				}
			} else {
				for (i in this.languages) {
					if (this.languages[i].tag == lang_tag) {

						this.languages[i].is_active = true
						set_cookie('lang', this.languages[i].tag, 30)

					} else this.languages[i].is_active = false
				}
			}
		},
		mounted: function(){
		},
	})

</script>
