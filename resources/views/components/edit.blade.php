<template id="template-edit">
	<div>
		<div class="flex flex-col gap-3 items-start">
            ...
            <h1 class="text-xl font-bold mb-5" v-if="id == 0">{{ __('Add new') }}</h1>
            <h1 class="text-xl font-bold mb-5" v-else-if="menu_item.table_name == 'orders'">{{ __('Client data') }}</h1> <!-- for tim -->
            <h1 class="text-xl font-bold mb-5" v-else>{{ __('Edit') }}</h1>
		</div>

		<div class="bg-white rounded-xl p-5 py-8 px-10 mb-5 flex flex-col gap-3" :class="menu_item.table_name">
		...
		<div :class="'editable-' + menu_item.table_name" v-for="(field, index) in fields" v-if="field.is_visible && field.relationship_count == 'editable'">
			<h1 class="text-lg font-bold mb-3" v-text="field.title"></h1>
        ...
        <div class="flex items-center gap-2">
			<button v-if="id == 0" class="bg-primary text-white text-sm font-bold px-4 py-2 rounded-lg cursor-pointer hover:bg-hover duration-300" v-on:click="save(true)">{{ __('Create') }}</button>
            <template v-else>
                <button v-on:click="save(false)" class="bg-primary text-white text-sm font-bold px-4 py-2 rounded-lg cursor-pointer hover:bg-hover duration-300">{{ __('Save changes') }}</button>
                <button v-on:click="save(true)" class="bg-primary text-white text-sm font-bold px-4 py-2 rounded-lg cursor-pointer hover:bg-hover duration-300">{{ __('Save and close') }}</button>
            </template>
        </div>
	</div>
</template>
