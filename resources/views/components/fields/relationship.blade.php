<script type="text/x-template" id="template-field-relationship">
	<div>
		...
		<div v-if="field.relationship_count == 'editable'">
			...
			<div class="btn btn-add" v-on:click="add_group" v-if="table_name != 'orders'">{{ __('proadmin.add_field') }} +</div>
		</div>
		...
		<div class="form-group" v-else>
			...
			<div>
				<div class="select-wrapper" v-if="field.relationship_count == 'single'">
					<v-select class="form-control" :options="[].concat.apply([{id: 0, title: '{{ __('proadmin.choose_select') }}'}], field.values)" label="title" :reduce="title => title.id" v-model="field.value"></v-select>
