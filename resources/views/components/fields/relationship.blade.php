	...
	<div>
		...
		<div v-if="field.relationship_count == 'editable'">
			<div class="flex flex-col gap-4">
				<div class="bg-white py-3 px-8 rounded-xl relative" v-for="(group, i) in field.value" :class="{active: isActive || table_name == 'orders'}">

					<div class="text-base font-bold cursor-pointer"
						v-text="group.fields[0].relationship_count != 'single' ? (group.fields[0].value ? group.fields[0].value : '1') : (group.fields[0].value_title ? group.fields[0].value_title : '2')"
						v-if="table_name != 'orders'"
						@click="isActive = !isActive"
					></div>

                    <div class="absolute top-3 right-3 flex items-center gap-2">
                        <div class="bg-red-500 text-white w-5 h-5 rounded-sm flex items-center justify-center cursor-pointer" v-on:click="field.value.splice(i, 1)" v-if="table_name != 'orders'">
                        ...
                        <div class="flex items-center justify-center bg-background rounded-sm w-5 h-5">
                            <svg :class="{'rotate-180': isActive}" class="w-2 h-2 duration-300" width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
					...
					<div v-show="isActive" class="pt-4">
			...
			<button class="my-4 border border-primary text-primary bg-light-grey hover:text-white w-full text-sm font-bold px-4 py-4 rounded-lg cursor-pointer hover:bg-hover duration-300" v-on:click="add_group" v-if="table_name != 'orders'">{{ __('Add') }} +</button>
		</div>
		...
		<div class="w-full" v-else>
			...
			<div class="w-full">
				<div class="w-full relative" v-if="field.relationship_count == 'single'">
					<v-select
                        class="w-full text-base px-4 py-2 appearance-none border border-stroke rounded-sm ring-0 focus:ring-0 outline-0 focus:outline-none"
                        :options="[].concat.apply([{id: 0, title: '{{ __('Choose select') }}'}], field.values)"
                        label="title"
                        :reduce="item => item.id"
                        v-model="field.value"
                    >
				...
				<div v-else-if="field.relationship_count == 'many'">
					<div class="relationship-many" >
						<div class="w-full relative">
							<v-select
                                class="w-full text-base px-4 py-2 appearance-none cursor-pointer border border-stroke rounded-sm ring-0 focus:ring-0 outline-0 focus:outline-none"
                                multiple
				...
				<div class="text-red-500 text-sm" v-text="error"></div>
			</div>
		</div>
	</div>
</script>
