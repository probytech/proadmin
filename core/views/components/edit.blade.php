	...
	<div class="edit">
		<div class="">
			...
			<div class="space-between">
				...
				<h1 v-if="id == 0">{{ __('proadmin.add_new') }}</h1>
				<h1 v-else-if="menu_item.table_name == 'orders'">{{ __('proadmin.client_data') }}</h1> <!-- for tim -->
				<h1 v-else>{{ __('proadmin.edit') }}</h1>
			</div>
		</div>
		...
		<div class="edit-fields-btns">
			<button v-if="id == 0" class="btn btn-primary" v-on:click="save(true)">{{ __('proadmin.create') }}</button>
			<template v-else>
				<button class="btn btn-primary" v-on:click="save(false)">{{ __('proadmin.save_adj') }}</button>
				<button class="btn btn-primary" v-on:click="save(true)">{{ __('proadmin.save_adj_close') }}</button>
			</template>
		</div>
	</div>
</template>
